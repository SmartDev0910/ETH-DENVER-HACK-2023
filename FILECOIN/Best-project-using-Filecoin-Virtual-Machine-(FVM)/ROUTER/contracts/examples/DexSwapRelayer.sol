...
contract DexSwapRelayer {
    ...
    event NewOrder(uint256 indexed _orderIndex, uint8 indexed _action);
    ...
    function orderLiquidityProvision(
        ...
        require(
            factory == dexSwapFactory || factory == uniswapFactory,
            "DexSwapRelayer: INVALID_FACTORY"
        );
        ...
        if (tokenA == address(0)) {
            ...
            require(
                ...
                "DexSwapRelayer: INSUFFICIENT_TOKEN_A"
            );
        }
        require(
            IERC20(tokenB).balanceOf(address(this)) >= amountB,
            "DexSwapRelayer: INSUFFICIENT_TOKEN_B"
        );
    ...
    function orderLiquidityRemoval(
        ...
        require(tokenA < tokenB, "DexSwapRelayer: INVALID_TOKEN_ORDER");
        require(
            amountA > 0 && amountB > 0 && liquidity > 0,
            "DexSwapRelayer: INVALID_LIQUIDITY_AMOUNT"
        );
        require(priceTolerance <= PARTS_PER_MILLION, "DexSwapRelayer: INVALID_TOLERANCE");
        require(block.timestamp <= deadline, "DexSwapRelayer: DEADLINE_REACHED");
        require(maxWindowTime > 30, "DexSwapRelayer: INVALID_WINDOWTIME");
    ...
    function executeOrder(uint256 orderIndex) external {
        ...
        require(
            ...
            "DexSwapRelayer: OBSERVATION_RUNNING"
        );
        ...
        if (order.action == PROVISION) {
            ...
            _unpool(tokenA, tokenB, pair, order.liquidity, minA, minB);
        }
        emit ExecutedOrder(orderIndex);
    }
    ...
    function updateOracle(uint256 orderIndex) external {
        ...
        require(
            !oracleCreator.isOracleFinalized(order.oracleId),
            "DexSwapRelayer: OBSERVATION_ENDED"
        );
    ...
    function _consultOracleParameters(
        ...
        if (reserveA > 0 && reserveB > 0) {
            uint256 poolStake = (amountA.add(amountB)).mul(PARTS_PER_MILLION) /
