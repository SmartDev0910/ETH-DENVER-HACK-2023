...
class Landing extends Component {
  ...
  buyZonuin = () => {
    ...
    const contractzonulet = new web3.eth.Contract(ZDEXabi, '0x2aFCbEc86045b348743D3dD9b1A2CA4b0ac59eB6') // ZonuletZDEX
    const contractfil = new web3.eth.Contract(cryptoAbi, '0x3467C3Ed08e806C12E713C3951139c29fb4946b5') // WETH on Filecoin Testnet

    const slippage = this.state.zonuamount 
    console.log("slip page", slippage)

    contractzonulet.methods.getEstimatedFILECOINforZONU(slippage.toString()).call()
      .then(receipt => {
        ...
        contractfil.methods.approve('0x2aFCbEc86045b348743D3dD9b1A2CA4b0ac59eB6', estone)
          ...
          .once('receipt', receipt => {
            ...
            contractzonulet.methods.zonulet(slippage.toString(), deadline).send({ from: this.state.account, value: estone }).once('receipt', async receipt => {
                ...
                console.log('Finishing FILECOIN to ZONU successfully!')
              })
              .catch(error => {

                toast.error("Transaction failed on the second step from FILECOIN!")
                this.setState({ txpend: false })
              })
          })
          .catch(error => {
            ...
            console.log(error)
            toast.error("Transaction failed on FILECOIN approval!")
            this.setState({ txpend: false })
          })
      })
      .catch(error => {
  ...
  changeAmount = amount => {
    if (amount > 0) {
      ...
      const contractzonulet2 = new web3.eth.Contract(zonuletZDEXabi2, '0x2aFCbEc86045b348743D3dD9b1A2CA4b0ac59eB6')
      ...
      contractzonulet2.methods.getEstimatedZONUforFILECOIN(web3.utils.toWei(amount.toString(), 'ether')).call()
        .then(receipt => {
  ...
  async like(e, owner, key) {
    ...
    const contractlike = new web3t.eth.Contract(abilike, '0xeb91A74782CE3f6C7C5DED926E3ea762d1dBBc0b')
  ...
  async ice(e, owner, key) {
    ...
    const web3one = new Web3('https://api.hyperspace.node.glif.io/rpc/v1')
    ...
    const addressb = '0x48a47b30A3e6B2edeA037d57172618DF86418931'
    const token_contract = new web3t.eth.Contract(abib, addressb)

    const pacmanprice = '1'

    token_contract.methods
      .approve('0xeb91A74782CE3f6C7C5DED926E3ea762d1dBBc0b', web3one.utils.toWei(pacmanprice, 'ether'))
      .send({ from: acct })
      .once('receipt', receipt => {
  ...
  async loadBlockchain() {
    ...
    const web3t = new Web3('https://api.hyperspace.node.glif.io/rpc/v1')
    ...
    setInterval(async () => {
      if (typeof web3 !== 'undefined' && this.state.showZONUModal === true) {
        const filecoinbalance = web3.utils.fromWei(await web3.eth.getBalance(this.state.account), 'ether')
        if (filecoinbalance > 0 && typeof filecoinbalance !== 'undefined') {
          this.setState({ filecoinbalance })

          document.getElementById('onebal').innerHTML = Number(filecoinbalance).toFixed(4)
      ...
      const cryptoAbi = wethAbi.abi
      const contractfil = new web3t.eth.Contract(cryptoAbi, '0x3467C3Ed08e806C12E713C3951139c29fb4946b5')

      const balFILlp = await contractfil.methods.balanceOf('0xb7617d9444617890fc23f428f3205c0d49952e6a').call() // DexSwap (FILECOIN - ZONU)
      const balDEZUBITlp = await contractfil.methods.balanceOf('0x1D4D5902118De52fD5e1f5E764abc3eD22aBadF0').call() //DexSwap (FILECOIN - USDC)
      const balUSDC = await contractfil.methods.balanceOf('0x635607BA3Db7FfC923f4d8f0AE9A356ebAEE864f').call() // DexSwap (FILECOIN - USDT)
      const balZonu = await contractfil.methods.balanceOf('0x0238AB191599e672d175D073FD1760C437f41571').call() // DexSwap (DEZU - ZONU)



      var combine = 0
      combine = balFILlp / 1e18
    ...
    const addv = '0xD1E7C945945a8Bf16E7aa1bcB18A98aa2159B94D' // Zonulet Verified
    const contractv = new web3t.eth.Contract(abiv, addv)

    const abia = ZonuletAvatars.abi
    const addr = '0x6ac6F871BA34B5Ce59EAF88D782b6e307207AC1B'
    ...
    const contractblack = new web3t.eth.Contract(abiblack, '0x5bee4C3E37B9f6dFc7E9da3bE48667651b96FA08')
      ...
      <div>
        ...
        >
          ...
          >
            <div align="center">
              ...
              Welcome to the <strong>Zonulet NFT Marketplace</strong>! This market is open and decentralized on the
                Filecoin Network. Anyone can mint NFTs and list them for sale. This results in possible fakes and
          ...
          >
            ...
            <h2 align="center">
              {/* TOOD: */}
            <img src="/filecoin_border.svg" border="0" height="86px" width="86px" /> 
            <i className="fa fa-arrow-right"></i>{' '}
            <img src="/zonu_border.svg" border="0" height="86px" width="86px" />
            </h2>
            <br />
            <div align="center">
              <p>
                <h3>
                  Swap your <strong>FILECOIN</strong> to <strong>ZONU</strong>
                </h3>
                <br />
                Balance{' '}
                <strong>
                  <span id="onebal">
                    {Number(this.state.filecoinbalance)
                      .toString()
                      .slice(
                        0,
                        Number(this.state.filecoinbalance)
                ...
                >
                  <input
                    ...
                    border="0"
                    id="zonuamount"
                    placeholder="Amount of FILECOIN/TFIL"
                    style={{ padding: '20px' }}
                  />
                  <br />
                  Will get you{' '}
              ...
              <div className="row">
                <div className="featuredContainer my-auto align-middle">
                  <div className="col-md-6 blockReact1">
                    ...
                    <strong><span className='txtGreen'> IPFS/FILECOIN</span>&nbsp;
                    </strong> <strong><span className='txtBlue'>BLOCKCHAIN </span></strong> technology to fetch NFT Prices, Storing NFT Data and more</span>
                    <div className="blockButton1">
                      ...
                      <a type="button" onClick={this.handleOpenZONUModal} className="actionGroup">
                        <span className="pointer zonuletBuy">
                          <span className="zonuletBordering trsLight">SWAP MY FILECOIN</span>
                        </span>
                      </a>
                    </div>
                    <div className="blockPowered" style={{display: 'block'}}>
                      <div className="poweredIconer">
                        ...
                        <span className='textWarning'>Filecoin Testnet Version</span>
                        </span>
                        </p>
                      </div>
                    </div>
