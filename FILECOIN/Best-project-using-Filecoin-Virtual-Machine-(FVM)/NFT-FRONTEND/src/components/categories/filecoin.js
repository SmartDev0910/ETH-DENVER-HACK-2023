    ...
    if (networkData) {
      ...
      fetch(`${process.env.REACT_APP_INFURA_HYPERSPACE}`, {
        ...
        .then(data => {
          ...
          const result = fuse.search('=Filecoin$')
          if (!result.length) {
            setData([])
            // setData(images);
          } else {
    ...
    <div>
      <div className="head-title col-auto mx-4">
        <h4 className="mb-0 font-weight-normal">
          All <strong>{images.length}</strong> "Filecoin" NFTs on Zonulet
        </h4>
      </div>
      <div className="container-fluid mb-5 explore-adj">
        <div className="row justify-content-around">
          <p align="center" className="text-secondary">
            This is where you can explore all "Filecoin" category NFTs that were minted within Zonulet NFT Market.
          </p>
          <p>
            <div align="center" style={{ maxWidth: '500px', margin: '0 auto' }}>
              <input
                placeholder="Search all 'Filecoin' on ZONU NFTs..."
                type="text"
