...
function Matic() {
  ...
  async function getNFTs() {
    ...
    if (networkData) {
      ...
      const contractlike = new web3.eth.Contract(abilike, '0x604950ba06Cb36D53193012E816E07296f995dD8')
     ...
     fetch(`${process.env.REACT_APP_INFURA_POLYGON}`, {
        ...
        .then(data => {
          ...
          const result = fuse.search('=Matic$')
          if (!result.length) {
            setData([])
            // setData(images);
          } else {
    ...
    <div>
      <div className="head-title col-auto mx-4">
        <h4 className="mb-0 font-weight-normal">
          All <strong>{images.length}</strong> "Matic" NFTs on Zonulet
        </h4>
      </div>
      <div className="container-fluid mb-5 explore-adj">
        <div className="row justify-content-around">
          <p align="center" className="text-secondary">
            This is where you can explore all "Matic" category NFTs that were minted within Zonulet NFT Market.
          </p>
          <p>
            <div align="center" style={{ maxWidth: '500px', margin: '0 auto' }}>
              <input
                placeholder="Search all 'Matic' ZONU NFTs..."
                type="text"
